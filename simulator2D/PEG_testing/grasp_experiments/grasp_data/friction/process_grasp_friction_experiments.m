...
function process_grasp_friction_experiments(  )

    % Load a ground truth
    sim = load('grasp_friction_groundTruth_PEG_b_1_0001.mat');
    sim_truth = sim.simExp; 
    h_truth = sim_truth.h; 
    ...
    %% STEWART-TRINKLE
    for hd = 10:14
        load(['grasp_friction_ST_b_1_' num2str(cell2mat(hdex(hd))) '.mat']);  
        sim = simExp; 
    ...
    for hd = [10:13 15 18]
        %try 
            sim = load(['grasp_friction_Pen_b_1_' num2str(cell2mat(hdex(hd))) '.mat']);  
            sim = sim.simExp; 
            ...
            hd_count = hd_count+1;
        %catch
        %    disp(['No such file: ' 'grasp_friction_Pen_b_1_' num2str(cell2mat(hdex(hd))) '.mat'])
        %end
    end
    
    %% PEG
    for hd = 10:19
        load(['grasp_friction_PEG_b_1_' num2str(cell2mat(hdex(hd))) '.mat']);  
        sim = simExp; 
